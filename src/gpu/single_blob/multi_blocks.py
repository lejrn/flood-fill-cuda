...
def optimized_flood_fill(img, visited, start_x, start_y, width, height, new_color, 
    ...
    while global_queue_front[0] < global_queue_rear[0]:
        ...
        for batch_start in range(warp_start, warp_end, 32):
            ...
            if queue_idx < warp_end and queue_idx < global_queue_rear[0]:
        ...
        if cuda.threadIdx.x == 0 and cuda.blockIdx.x == 0:
            global_queue_front[0] = global_queue_rear[0]
...
def setup_scene():
    ...
    # Create a red blob (~100x100) in the image using random walk
    blob_center_x = random.randint(1000, 3000)
    blob_center_y = random.randint(1000, 3000)
    num_walks = 10
    walk_length = 10000
...
def profile_kernel(num_runs=10, explore_configs=False):
    """
    Profile the optimized flood fill kernel performance.
    
    Args:
        num_runs: Number of times to run the kernel for averaging
        explore_configs: Whether to test different configurations
    
    Returns:
        The processed image and visited array from the last run
    """
    print("=== Optimized Multi-SM Flood Fill Performance ===")
    print(f"RTX 4060 Configuration:")
    print(f"  - 24 SMs, 144 blocks total (6 blocks per SM)")
    print(f"  - 256 threads per block (8 warps per block)")
    print(f"  - 1,152 total warps across GPU")
    print(f"  - Global memory queue (L2 cached)")
    print()
    
    # Allocate global memory arrays for the queue
