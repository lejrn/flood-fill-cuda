...
def is_red(img, x, y, width, height):
    ...
    if x < 0 or x >= width or y < 0 or y >= height:
        return False
    return img[y, x, 0] == 255 and img[y, x, 1] == 0 and img[y, x, 2] == 0
...
def flood_fill(img, visited, start_x, start_y, width, height, new_color):
    ...
    # Initialization using 2D indexing for visited.
    if tid == 0:
        queue_x[0] = start_x
        queue_y[0] = start_y
        queue_front[0] = 0
    ...
    while queue_front[0] < queue_rear[0]:
        ...
        for idx in range(start_idx, end_idx):
            ...
            img[y, x, 0] = new_color[0]
            img[y, x, 1] = new_color[1]
            img[y, x, 2] = new_color[2]
                ...
                if is_valid_pixel(nx, ny, width, height):
                    if is_not_visited(visited, nx, ny, width):
                        if is_red(img, nx, ny, width, height):
                            # Use cuda.atomic.cas on the 2D visited array.
                            old = cuda.atomic.cas(visited, (ny, nx), 0, 1)
                            if old == 0:
        ...
        if tid == 0:
            # In previous designs we advanced the front by blockDim.x to skip exactly one batch per iteration.
            # Here we set it to queue_rear[0] because all queue items have been processed.
...
def main():
    ...
    # Launch kernel using one block and 256 threads.
    ...
    axs[0].scatter([start_x], [start_y], color='green', s=50, marker='o')
    axs[0].text(start_x+5, start_y+5, "Start", color='green')
    axs[1].imshow(visited_result, cmap='gray')
    axs[1].set_title("Visited Pixels")
    axs[1].scatter([start_x], [start_y], color='red', s=50, marker='x')
    plt.show()
